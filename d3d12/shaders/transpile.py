...
#
# Hand-written HLSL in xbox/ (decoder kernels for the moonlight-xbox client, tuned for
# the Xbox's hevcPlayback GPU partition; see xbox_kernel_model in
# pyrowave_d3d12_device.cpp) is compiled as-is, twice: <name>_sm64 for Shader Model
# 6.4 devices whose waves are always 64 lanes (Xbox Series), the build that ships, and
# <name>_sm66 with [WaveSize(64)] to test them on a PC GPU. Everything else uses the
# translated kernels.
...
# (output name, hand-written HLSL in xbox/, dxc defines)
XBOX_SHADERS = [
	('wavelet_dequant_xbox', 'wavelet_dequant.hlsl', []),
	('idwt_xbox', 'idwt.hlsl', ['DC_SHIFT=0']),
	('idwt_xbox_dc', 'idwt.hlsl', ['DC_SHIFT=1']),
]
...
def main():
	...
	try:
		...
		for name, src, src_defines in XBOX_SHADERS:
			for suffix, model, defines in (('sm64', 'cs_6_4', src_defines),
			                               ('sm66', 'cs_6_6', src_defines + ['PW_WAVE_SIZE_ATTRIBUTE=[WaveSize(64)]'])):
				dxil = tmp / f'{name}_{suffix}.dxil'
				run([DXC, '-nologo', '-T', model, '-E', 'main', '-O3', '-Qstrip_debug', '-Qstrip_reflect'] +
				    [f'-D{d}' for d in defines] + [win_path(XBOX_DIR / src), '-Fo', win_path(dxil)])
				blobs.append((f'{name}_{suffix}', dxil.read_bytes()))
				print(f'  {name}_{suffix}: {len(blobs[-1][1])} bytes DXIL')
