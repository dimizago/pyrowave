...
// Profile desktop; glslc defines: FP16=0 PRECISION=2; dxc defines: SPIRV_CROSS_CONSTANT_ID_0=true
...
cbuffer Registers
{
    int2 _121_resolution : packoffset(c0);
    float2 _121_inv_resolution : packoffset(c0.z);
    int2 _121_aligned_resolution : packoffset(c1);
};
...
groupshared float2 shared_block[20][41];
...
float2 generate_mirror_uv(inout int2 coord)
{
    ...
    coord += int2(1, 1);
    int2 end_mirrored_clamp = (int2(2, 2) * _121_aligned_resolution) - _121_resolution;
    int2 past_wrapped_coord = (coord + (int2(2, 2) * (_121_resolution - _121_aligned_resolution))) + int2(1, 1);
    int2 _145 = min(coord, _121_resolution);
    bool2 _149 = bool2(coord.x >= end_mirrored_clamp.x, coord.y >= end_mirrored_clamp.y);
    coord = int2(_149.x ? past_wrapped_coord.x : _145.x, _149.y ? past_wrapped_coord.y : _145.y);
    return float2(coord) * _121_inv_resolution;
}

void store_shared(uint y, uint x, float2 v)
{
    shared_block[y][x] = float2(v);
}
...
void load_image_with_apron()
{
    ...
    int2 param_1 = coord0;
    float2 _195 = generate_mirror_uv(param_1);
    float4 texels0 = float4(uTexture.GatherRed(_uTexture_sampler, float3(_195, 0.0f))).wzxy;
    int2 param_2 = coord0 + int2(16, 0);
    float2 _215 = generate_mirror_uv(param_2);
    float4 texels1 = float4(uTexture.GatherRed(_uTexture_sampler, float3(_215, 0.0f))).wzxy;
    int2 param_3 = coord0 + int2(0, 16);
    float2 _232 = generate_mirror_uv(param_3);
    float4 texels2 = float4(uTexture.GatherRed(_uTexture_sampler, float3(_232, 0.0f))).wzxy;
    int2 param_4 = coord0 + int2(16, 16);
    float2 _249 = generate_mirror_uv(param_4);
    float4 texels3 = float4(uTexture.GatherRed(_uTexture_sampler, float3(_249, 0.0f))).wzxy;
    if (DCShift)
    {
        texels0 -= 0.5f.xxxx;
        texels1 -= 0.5f.xxxx;
    ...
    float2 _408 = generate_mirror_uv(param_29);
    float4 texels = float4(uTexture.GatherRed(_uTexture_sampler, float3(_408, 0.0f))).wzxy;
    if (DCShift)
    {
        texels -= 0.5f.xxxx;
    }
    ...
    float2 _470 = generate_mirror_uv(param_36);
    float4 texels_1 = float4(uTexture.GatherRed(_uTexture_sampler, float3(_470, 0.0f))).wzxy;
    if (DCShift)
    {
        texels_1 -= 0.5f.xxxx;
    }
    ...
    if (local_index < 16u)
    {
        ...
        float2 _536 = generate_mirror_uv(param_43);
        float4 texels_2 = float4(uTexture.GatherRed(_uTexture_sampler, float3(_536, 0.0f))).wzxy;
        if (DCShift)
        {
            texels_2 -= 0.5f.xxxx;
        }
...
void comp_main()
{
    ...
    {
        int _1050 = local_coord.x * 2;
        for (int x = _1050; x < 32; x += 16)
        {
            uint param_3 = uint(y);
